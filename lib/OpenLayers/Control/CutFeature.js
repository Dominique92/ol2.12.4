...
OpenLayers.Control.CutFeature = OpenLayers.Class(OpenLayers.Control.ModifyFeature, {
    ...
    /**
     * APIProperty: summit
     * {Boolean} Supprime les sommets si on clique dessus
     */
	summit: true,

    /**
     * APIProperty: split
     * {Boolean} Coupe une trace en 2 si on clique sur les ronds du milieu des côtés
     */
	split: true,
    ...
    initialize: function (options) {
		...
		if (!this.summit)
			this.CLASS_NAME = 'OpenLayers.Control.CutFeatureSplit';
		if (!this.split) {
			this.CLASS_NAME = 'OpenLayers.Control.CutFeatureSummit';
			this.displayClass = 'olControlCutFeatureSummit';
		}
	},
		...
		if(this.feature) {
				...
				vertex.geometry.parent)
			{
				...
				// Si c'est un point virtuel (au milieu d'un segment), on coupe de la ligne en 2 segments
					...
					OpenLayers.Util.indexOf(this.virtualVertices, vertex) != -1) {
